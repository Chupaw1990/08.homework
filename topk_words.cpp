...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cerr << "Usage: topk_words [FILES...]" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    std::cout << "Quantity of streams " << threads_number << std::endl
              << std::endl;
    ...
    for (auto& cntptr : counter_ptrs) {
        if (final_dictionary.empty()) {
            ...
            for (auto it = cntptr->begin(); it != cntptr->end(); ++it) {
                try {
    ...
    std::cout << "Elapsed time is " << elapsed_ms.count() << " ms" << std::endl;
}
...
void print_topk(std::ostream& stream, const Counter& counter, const size_t k) {
    ...
    std::for_each(
        ...
        [&stream](const Counter::const_iterator& pair) {
            stream << std::setw(4) << pair->second << " " << pair->first << std::endl;
        });
}

bool dictionary_contains(Counter* cnt_ptr, const std::string& str) {
    for (auto it = cnt_ptr->begin(); it != cnt_ptr->end(); ++it) {
        if (it->first == str) {
            return true;
        }
    }
